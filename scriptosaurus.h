...
static int				_ssr_run(char* cmd, _ssr_str_t* outerr);
...
static int	_ssr_run(char* cmd, _ssr_str_t* outerr)
{
    ...
    if (outerr != NULL) {
        outerr->b = _ssr_alloc(out_size + err_size + 2);
        if (out_size) ReadFile(stdout_r, outerr->b, out_size, &out_size, NULL);
        if (out_size && err_size) outerr->b[out_size++] = '\n';
        if (err_size) ReadFile(stderr_r, outerr->b + out_size, err_size, &err_size, NULL);
        outerr->b[out_size + err_size] = '\0';
    }
...
static bool _ssr_compile_msvc(const char* input, ssr_config_t* config, const char* _out, int stages)
{
	...
	if (stages & _SSR_COMPILE)
	{
        ...
        _ssr_log(SSR_CB_INFO, compile.b);
        
        _ssr_str_t output;
        int ret = _ssr_run(compile.b, &output) == 0;
		_ssr_str_destroy(compile);
        //if (strlen(output.b) > 0) {
            if (ret) {
                _ssr_log(SSR_CB_WARN, output.b);
            } else {
                _ssr_log(SSR_CB_ERR, output.b);
            }
        //}
        _ssr_str_destroy(output);
	...
	if (stages & _SSR_LINK)
	{
        ...
        _ssr_log(SSR_CB_INFO, link.b);

        _ssr_str_t output;
        int ret = _ssr_run(link.b, &output) == 0;
...
static bool _ssr_compile_clang(const char* input, ssr_config_t* config, const char* _out, int stages)
{
	...
	if (stages & _SSR_COMPILE)
	{
		...
		_ssr_str_t output;
		int ret = _ssr_run(compile.b, &output);
		_ssr_str_destroy(compile);
		_ssr_str_destroy(output);
		if (ret != 0)
			goto end;
	...
	if (stages & _SSR_LINK)
	{
		...
		_ssr_str_t output;
		int ret = _ssr_run(link.b, &output);
		_ssr_str_destroy(link);
		_ssr_str_destroy(output);
		if (ret != 0)
			goto end;
